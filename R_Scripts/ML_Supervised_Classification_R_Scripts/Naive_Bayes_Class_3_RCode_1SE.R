...
#Import data
HSIOil<-read_excel("SG_0D_A.xlsx")
...
CC_k_class_3_train_MSC_SG_2D<-as.data.frame(fit_naive_bayes$results)
write.csv(CC_k_class_3_train_MSC_SG_2D, file = "CC_k_class_3_train_SG_0D.csv")
...
CC_k_class_3_train_SG_0D<-as.data.frame(fit_naive_bayes_1$results)
write.csv(CC_k_class_3_train_SG_0D, file = "CC_k_class-3_train_SG_0D_A_1.csv")
#------------------------END OF TRAINING----------------------------------------
#Test/Prediction  on the External Validation Set

#Test the model (External validation) with the optimal value k
test_model_class_3<-predict(fit_naive_bayes_1,newdata = 
                              data_testPreprocessed[,-1], y = data_testPreprocessed[,1])

#Get the confusion matrix for test data    
cf_class_3_test<-confusionMatrix(test_model_class_3,data_testPreprocessed$class_3)
print(cf_class_3_test)
#Get and save metrics table for testing
test_metric_class_3_SG_0D_A<-as.data.frame(cf_class_3_test$byClass)
write.csv(test_metric_class_3_SG_0D_A,file = "test_metric_class_3_SG_0D_A.csv")

#Get and save confusion matrix table for testing
cf_table_class_3_test_SG_0D_A<-as.data.frame(cf_class_3_test$table)
cf_table_class_3_test_SG_0D_A<-write.csv(cf_table_class_3_test_SG_0D_A, 
                                       file = "cf_table_class_3_test_SG_0D_A.csv")
print(cf_class_3_test)
...
write.csv(check_misclas,file = "check_misclass_table_SG_0D.csv",row.names = FALSE)
#-------------------------------------------------------------------------------
