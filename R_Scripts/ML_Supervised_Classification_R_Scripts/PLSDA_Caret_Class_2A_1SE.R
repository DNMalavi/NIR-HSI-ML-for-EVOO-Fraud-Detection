...
CC_k_class_2_train_Raw_A<-as.data.frame(fit_pls$results)
write.csv(CC_k_class_2_train_Raw_A, file = "CC_k_class_2_train_Raw_A.csv")

##Find important variables from PLS model
impo_variable<-varImp(fit_pls)
ImptV<-(impo_variable$importance)
write.csv(ImptV, file = "important_variable_Raw_class_2.csv")

##Use the optimal model to predict the external samples
#predict the test set
test_class_2<-predict(fit_pls,newdata = data_test[,-225],y = data_test$class_2,ncomp=ideal_LVs)
...
cf_class_2_test_Raw_A<-as.data.frame(cf_class_2_test$table)
write.csv(cf_class_2_test_Raw_A, 
          "cf_class_2_test_Raw_A.csv")

##Save the testing metrics
cf_class_2_test_metrics_Raw_A<-as.data.frame(cf_class_2_test$byClass)
write.csv(cf_class_2_test_metrics_Raw_A,file = "cf_class_2_test_metrics_Raw_A.csv")
#------------------------------------------------------------------------------
#Save the observed and predicted csv file
check_misclas<-as.data.frame(cbind(data_test$class_2,test_class_2))
colnames(check_misclas)<-c("observed","predicted")
write.csv(check_misclas,file = "check_misclass_table_Raw_A.csv",row.names = FALSE)
#-----------------------THE END-------------------------------------------------
...
columns_Raw_class_2<-HSIOil  %>% filter(Cal_Val == 1) %>% 
  select(4,8)
table(columns_Raw_class_2[,])
#Check the pls model 
table(columns_Raw_class_2[,1])
#get the scores and check proportions
fit_pls$finalModel$loadings

scores_Raw_class2<-as.data.frame.matrix(fit_pls$finalModel$scores)
scores_Raw_class2

scores_1<-cbind(columns_Raw_class_2,scores_Raw_class2)
scores_1<-clean_names(scores_1)
head(scores_1)


Raw_plsda_plot_class2 <- scores_1 %>%
  ggplot(mapping = aes(x = comp_2, y = comp_5, color = class_2, 
                       shape = class_2)) +
  geom_point(size = 2.5) +
  theme_bw() +
  labs(x = 'LV2', y = 'LV5') +
  scale_color_manual(values = c("black", "#330000", "blue")) +
  scale_shape_manual(values = c(18, 8, 17)) +
  ...
  annotate("text",x = 0.0, y = 0.035, label = "(e) SG Smoothing + PLS-DA", color = "black", size =3)

Raw_plsda_plot_class2                 
ggsave("output_Raw_plsda_plot_class2.png", plot = Raw_plsda_plot_class2, dpi = 300)

###Latent Variables
##Latent Variable Plot
pls<-fit_pls$results
x_Lvs= 9
y_Acc = 0.993895994858677
...
text(x = 100, y = 0.99, labels = "(b)", col = "black", cex = 0.8)
...
ggplot(data = as.data.frame(cf_class_2_test$table), aes(x = Reference, y = Prediction)) +
  ...
  scale_fill_gradient(low = "white", high = "tan", name = "Frequency") +
  theme_minimal() +
  labs(x = "Actual Class", y = "Predicted Class", color = 'black')+
  theme(legend.position =  "none")+
  annotate("text",x = 3.4, y = 3.4, label = "(d)", color = "black")+theme(
