...
HSIOil<-HSIOil %>% select(c(1:11,90:92,139:145,152:159))
...
fit_pls<-caret::train(y = data_train[,21], x = data_train[,-21], method = "pls", 
...
CC_k_class_3_train_Raw_A<-as.data.frame(fit_pls$results)
...
#-----------------------THE END-------------------------------------------------
colnames(pred)
pred<-as.data.frame(fit_pls$pred)
pred<-pred[,c(2,3)]
table(pred)

p1<-pred %>% ggplot(aes(x = obs, y = pred))+geom_point()
p1
fit_pls$finalModel$coefficients
