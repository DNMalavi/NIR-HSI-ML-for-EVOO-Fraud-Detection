...
HSIFull<-as.matrix(HSIOil[,c(26:27,32,34:35,37,42:45,132:134,147:148,182:186)])
class(HSIOil)
class(HSIFull)
...
rf_model_1<-randomForest(y = data_train[,21], x = data_train[,-21], 
                       proximity = TRUE)
...
rf_model_tune_2<- tuneRF(data_train[,-21], data_train[,21],
                         stepFactor = 0.5, improve = 0.05, 
...
write.csv(results_df, "tuning_class_3_SG_2D_A.csv", row.names = FALSE)
...
CV_class_3_train_SG_2D_A <- as.data.frame(final_model$results)
CV_class_3_train_SG_2D_A
write.csv(CV_class_3_train_SG_2D_A, file = "CV_class_3_train_SG_2D_A.csv")
#----END OF TRAINING AND CROSS-VALIDATION--------------------------------------

#-----------------#TESTING THE MODEL ON EXTERNAL DATA SET-----------------------
#predict the test set
test_class_3<-predict(final_model,newdata = data_test[,-21],y = data_test$class_3)
