...
CC_k_class_1_train_MSC_SG_0D_A<-as.data.frame(fit_svm$results)
write.csv(CC_k_class_1_train_MSC_SG_0D_A, file = "CC_k_class_1_train_MSC_SG_0D_A.csv")

##Find important variables from SVM model
impo_variable<-varImp(fit_svm)
ImptV<-(impo_variable$importance)
write.csv(ImptV, file = "important_variable_MSC_SG_0D_class_1.csv")

#------------------END OF TRAINING AND CROSS-VALIDATION-------------------------

##Use the optimal model to predict the external samples
#predict the test set
test_class_1<-predict(fit_svm,newdata = data_test[,-225],y = data_test$class_1)
...
cf_class_1_test_MSC_SG_0D_A<-as.data.frame(cf_class_1_test$table)
write.csv(cf_class_1_test_MSC_SG_0D_A, 
          "cf_class_1_test_MSC_SG_0D_A.csv")

##Save the testing metrics
cf_class_1_test_metrics_MSC_SG_0D_A<-as.data.frame(cf_class_1_test$byClass)
write.csv(cf_class_1_test_metrics_MSC_SG_0D_A,file = "cf_class_1_test_metrics_MSC_SG_0D_A.csv")
#-------------------------END---------------------------------------------------