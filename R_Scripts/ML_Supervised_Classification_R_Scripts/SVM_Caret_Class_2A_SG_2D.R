...
#Train the SVM model using cross-validation (10 folds, 10 iterations)
...
ideal_SVs<-fit_svm$bestTune# to be used for the test set results

#Find number of SVs
fit_svm$finalModel

#Save CV SVM model results 
CC_k_class_2_train_SNV_SG_2D_A<-as.data.frame(fit_svm$results)
write.csv(CC_k_class_2_train_SNV_SG_2D_A, file = "CC_k_class_2_train_SNV_SG_2D_A.csv")

##Find important variables from SVM model
impo_variable<-varImp(fit_svm)
ImptV<-(impo_variable$importance)
write.csv(ImptV, file = "important_variable_SNV_SG_2D_class_2.csv")

#------------------END OF TRAINING AND CROSS-VALIDATION-------------------------

##Use the optimal model to predict the external samples
#predict the test set
test_class_2<-predict(fit_svm,newdata = data_test[,-225],y = data_test$class_2)
...
cf_class_2_test_SNV_SG_2D_A<-as.data.frame(cf_class_2_test$table)
write.csv(cf_class_2_test_SNV_SG_2D_A, 
          "cf_class_2_test_SNV_SG_2D_A.csv")

##Save the testing metrics
cf_class_2_test_metrics_SNV_SG_2D_A<-as.data.frame(cf_class_2_test$byClass)
write.csv(cf_class_2_test_metrics_SNV_SG_2D_A,file = "cf_class_2_test_metrics_SNV_SG_2D_A.csv")
#-----------------------THE END-------------------------------------------------
