...
HSIFull<-as.matrix(HSIOil[,c(26:28,31:36,38,42:45,131:132,183:186)])
class(HSIOil)
class(HSIFull)
...
sigma_values <- c(0.1,0.09,0.08,0.07,0.06, 0.05,0.04, 0.03, 0.02, 0.01)
C_values <- c(0.05, 0.1, 0.5, 1, 5, 10, 25, 50, 100,150, 200,250, 300, 350,400)
...
metric<-"Accuracy"
fit_svm<-caret::train(y = data_train[,21], x = data_train[,-21], 
                      method = "svmRadial", trControl = control, tuneGrid = grid)

print(fit_svm)#model summary
plot(fit_svm,pch = 1, xlab = "cost", ylab = "Accuracy (RepeatedCV)")

#Find the optimum number of sigma and cost from tuning
print(fit_svm$bestTune)

#Find number of SVs
print(fit_svm$finalModel)

#Save CV SVM model results 
CC_k_class_3_train_SG_2D_A<-as.data.frame(fit_svm$results)
write.csv(CC_k_class_3_train_SG_2D_A, file = "CC_k_class_3_train_SG_2D_A.csv")

##Find important variables from SVM model
impo_variable<-varImp(fit_svm)
ImptV<-(impo_variable$importance)
write.csv(ImptV, file = "important_variable_SG_2D_class_3.csv")

#------------------END OF TRAINING AND CROSS-VALIDATION-------------------------

##Use the optimal model to predict the external samples
#predict the test set
test_class_3<-predict(fit_svm,newdata = data_test[,-21],y = data_test$class_3)
...
cf_class_3_test_SG_2D_A<-as.data.frame(cf_class_3_test$table)
write.csv(cf_class_3_test_SG_2D_A, 
          "cf_class_3_test_SG_2D_A.csv")

##Save the testing metrics
cf_class_3_test_metrics_SG_2D_A<-as.data.frame(cf_class_3_test$byClass)
write.csv(cf_class_3_test_metrics_SG_2D_A,file = "cf_class_3_test_metrics_SG_2D_A.csv")
#-----------------------THE END-------------------------------------------------
